    ...
    (function() {
      ...
      Howl2.prototype = {
        ...
        play: function(sprite, internal) {
          ...
          var sound = id3 ? self._soundById(id3) : self._inactiveSound();
          if (!sound) {
            return null;
          }
          if (id3 && !sprite) {
        ...
        volume: function() {
          ...
          if (typeof vol !== "undefined" && vol >= 0 && vol <= 1) {
            ...
            sound = id3 ? self._soundById(id3) : self._sounds[0];
        ...
        fade: function(from, to, len, id3) {
          ...
          for (var i = 0; i < ids.length; i++) {
            ...
            if (sound) {
              ...
              self._startFadeInterval(sound, from, to, len, ids[i], typeof id3 === "undefined");
            }
          }
          return self;
        },
         ...
         */
        _startFadeInterval: function(sound, from, to, len, id3, isGroup) {
          var self = this;
          var vol = from;
          var diff = to - from;
            ...
            for (var i = 0; i < id3.length; i++) {
              ...
              if (sound) {
                if (self.playing(id3[i])) {
                  sound._rateSeek = self.seek(id3[i]);
                ...
                var timeout = duration * 1e3 / Math.abs(sound._rate);
                if (self._endTimers[id3[i]] || !sound._paused) {
                  self._clearTimer(id3[i]);
                  self._endTimers[id3[i]] = setTimeout(self._ended.bind(self, sound), timeout);
                }
                self._emit("rate", sound._id);
              }
            }
          } else {
            sound = self._soundById(id3);
        ...
        seek: function() {
          ...
          if (sound) {
            if (typeof seek === "number" && seek >= 0) {
              ...
              if (self._webAudio) {
                var realTime = self.playing(id3) ? Howler2.ctx.currentTime - sound._playStart : 0;
        ...
        _clearTimer: function(id3) {
          ...
          if (self._endTimers[id3]) {
            if (typeof self._endTimers[id3] !== "function") {
              ...
              var sound = self._soundById(id3);
              if (sound && sound._node) {
                sound._node.removeEventListener("ended", self._endTimers[id3], false);
              }
            }
...
__export(main_exports, {
  VIEW_TYPE_LIFE_RPG: () => VIEW_TYPE_LIFE_RPG,
  default: () => LifeRPGPlugin
});
...
var SOURCE_MAPPING = {
  ...
  "Focused study time (minutes)": { skillId: "INTELLECT", xpPerUnit: 2, type: "DURATION" },
  // 120xp/hr
  "Work Quality": { skillId: "DISCIPLINE", xpPerUnit: 50, type: "RATING" },
  // Multiplier? Assuming 1-5 scale
  // Modern (2023-2025)
...
var parseVault = async (app, files, defaultXp = 5, customMappings = {}) => {
...
var parseFileContent = async (app, file, skillXpMap, unknownSources, customMappings, tasks) => {
  ...
  while ((taskMatch = taskRegex.exec(content)) !== null) {
    ...
    const skillTag = skillTagMatch ? skillTagMatch[1] : void 0;
    const id3 = `${file.basename}-${taskMatch.index}`;
    tasks.push({
      id: id3,
      description,
      completed,
      xpReward: completed ? 10 : 0,
      // Dynamic later
...
var processFrontmatter = (frontmatter, skillXpMap, unknownSources, customMappings) => {
  for (const key in frontmatter) {
    processSource(key, frontmatter[key], skillXpMap, unknownSources, customMappings);
  }
};
var processSource = (sourceKey, value, skillXpMap, unknownSources, customMappings) => {
  if (["tags", "cssclasses", "date", "aliases", "position"].includes(sourceKey.toLowerCase()))
    return;
...
var HABIT_DEFINITIONS = [
  ...
  // SOCIAL
  { key: "Call_loved_one", skillId: "social", xpPerUnit: 50, unit: "call", target: 1, label: "Call Family", icon: "fa-phone", isBinary: true },
  // Usually 1 call?
...
var SKILL_ICONS = Object.values(SKILL_DEFINITIONS).reduce((acc, s) => ({ ...acc, [s.name]: s.icon }), {});
var SKILL_COLORS = Object.values(SKILL_DEFINITIONS).reduce((acc, s) => ({ ...acc, [s.name]: s.color }), {});
var SKILL_ID_TO_NAME = Object.values(SKILL_DEFINITIONS).reduce((acc, s) => ({ ...acc, [s.id]: s.name }), {});
var GARDEN_THEMES = [
  // Standard
...
var DATA_FILE_PATH = "life-rpg-data.json";
var DEFAULT_DATA = {
  baseLayout: [
...
var saveGameData = async (plugin, data) => {
  try {
...
// components/HistoryCalendar.tsx
var import_jsx_runtime = __toESM(require_jsx_runtime());
var HistoryCalendar = ({ stats, onDayClick }) => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bg-white rounded-xl shadow-sm border border-stone-200 p-8 w-full overflow-x-auto", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between mb-6", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", { className: "text-xl font-light text-stone-700 flex items-center gap-2", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "fa-solid fa-calendar-days text-stone-400" }),
        "Chronicle of Growth"
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-xs font-mono text-stone-400 uppercase tracking-widest", children: "Last 12 Months" })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "grid grid-rows-7 grid-flow-col gap-1 w-max mx-auto", children: allDays.map((dateObj) => {
      const dateStr = dateObj.toISOString().split("T")[0];
      const stat = stats.find((s) => s.date === dateStr);
      const xp = (stat == null ? void 0 : stat.totalXp) || 0;
      return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max bg-stone-800 text-white p-2 rounded text-xs z-50 hidden group-hover:block pointer-events-none shadow-lg", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "font-bold", children: dateStr }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                xp.toLocaleString(),
                " XP"
              ] }),
              (stat == null ? void 0 : stat.primarySkill) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-stone-400 italic", children: stat.primarySkill })
            ] })
          ]
        },
        dateStr
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-8 flex justify-end items-center gap-2 text-[10px] text-stone-400 font-mono uppercase tracking-widest", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Less" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex gap-1", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-[2px] bg-stone-100/50" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-[2px] bg-emerald-100" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-[2px] bg-emerald-300" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-[2px] bg-emerald-500" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-[2px] bg-emerald-700" })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "More" })
    ] })
  ] });
};
var HistoryCalendar_default = HistoryCalendar;

// components/Chronicle.tsx
var import_jsx_runtime2 = __toESM(require_jsx_runtime());
var Chronicle = ({ stats, onDayClick }) => {
  const statsArray = stats ? Object.values(stats) : [];
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "max-w-4xl mx-auto space-y-8", children: [
    /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "text-center", children: [
      /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("h2", { className: "text-3xl font-light text-stone-800 mb-2", children: "The Chronicle" }),
      /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-stone-400 italic font-serif", children: '"A thousand miles begins with a single step."' })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(HistoryCalendar_default, { stats: statsArray, onDayClick })
  ] });
};
var Chronicle_default = Chronicle;
...
var NativeAnimationExtended = class extends NativeAnimation {
  ...
  updateMotionValue(value) {
    ...
    const sampleTime = secondsToMilliseconds((_a = this.finishedTime) != null ? _a : this.time);
...
function PopChild({ children, isPresent, anchorX, root }) {
  ...
  return (0, import_jsx_runtime3.jsx)(PopChildMeasure, { isPresent, childRef: ref, sizeRef: size, children: React2.cloneElement(children, { ref: composedRef }) });
}
...
function buildSVGAttrs(state, {
  ...
  if (style.transform) {
    style.transformBox = (_b = styleProp == null ? void 0 : styleProp.transformBox) != null ? _b : "fill-box";
...
function useRender(Component4, props, ref, { latestValues }, isStatic, forwardMotionProps = false) {
  ...
  const elementProps = Component4 !== import_react16.Fragment ? { ...filteredProps, ...visualProps, ref } : {};
  const { children } = props;
  const renderedChildren = (0, import_react16.useMemo)(() => isMotionValue(children) ? children.get() : children, [children]);
  return (0, import_react16.createElement)(Component4, {
    ...elementProps,
...
function createMotionComponent(Component4, { forwardMotionProps = false } = {}, preloadedFeatures, createVisualElement) {
  ...
  const useVisualState = isSVGComponent(Component4) ? useSVGVisualState : useHTMLVisualState;
  function MotionDOMComponent(props, externalRef) {
    ...
    return (0, import_jsx_runtime6.jsxs)(MotionContext.Provider, { value: context, children: [MeasureLayout2 && context.visualElement ? (0, import_jsx_runtime6.jsx)(MeasureLayout2, { visualElement: context.visualElement, ...configAndProps }) : null, useRender(Component4, props, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, forwardMotionProps)] });
  }
  MotionDOMComponent.displayName = `motion.${typeof Component4 === "string" ? Component4 : `create(${(_b = (_a = Component4.displayName) != null ? _a : Component4.name) != null ? _b : ""})`}`;
...
var createDomVisualElement = (Component4, options) => {
  return isSVGComponent(Component4) ? new SVGVisualElement(options) : new HTMLVisualElement(options, {
    allowProjection: Component4 !== import_react22.Fragment
  });
};
...
function MeasureLayout(props) {
  ...
  return (0, import_jsx_runtime7.jsx)(MeasureLayoutWithContext, { ...props, layoutGroup, switchLayoutGroup: (0, import_react23.useContext)(SwitchLayoutGroupContext), isPresent, safeToRemove });
}
...
var TaskList = ({ tasks, onToggleTask, title, isHabitList = false, iconUrl }) => {
  return /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "mb-12", children: [
    /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "flex items-center justify-between mb-6 px-4", children: [
      /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("h3", { className: "text-stone-800 font-serif italic text-xl flex items-center gap-3", children: [
        iconUrl ? /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("img", { src: iconUrl, alt: "icon", className: "w-5 h-5 object-contain opacity-40" }) : /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("i", { className: `fa-solid ${isHabitList ? "fa-leaf" : "fa-feather-pointed"} text-stone-300` }),
        title
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("span", { className: "text-[10px] text-stone-400 font-bold uppercase tracking-[0.2em] bg-stone-50 px-3 py-1 rounded-full border border-stone-100", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "space-y-3", children: [
      /* @__PURE__ */ (0, import_jsx_runtime8.jsx)(AnimatePresence, { mode: "popLayout", children: tasks.map((task) => {
        const isHabit = task.isHabit;
        const progress2 = isHabit && task.targetValue ? Math.min(100, (task.currentValue || 0) / task.targetValue * 100) : task.completed ? 100 : 0;
        return /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)(
          motion.div,
          {
            ...
            className: `group relative flex items-center p-4 rounded-2xl border transition-all duration-300 cursor-pointer
                    ${task.completed ? "bg-stone-50/50 border-stone-100/50" : "bg-white border-stone-100 hover:border-stone-300 shadow-sm hover:shadow-md hover:-translate-y-0.5"}`,
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("div", { className: "mr-6 flex-shrink-0", children: isHabit ? /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("div", { className: `w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors
                        ${task.completed ? "bg-stone-900 border-stone-900" : "border-stone-100 bg-stone-50"}`, children: /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("i", { className: `fa-solid ${task.completed ? "fa-check text-white" : "fa-rotate text-stone-300"} text-[10px]` }) }) : /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("div", { className: `w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all
                        ${task.completed ? "bg-stone-900 border-stone-900" : "border-stone-200 group-hover:border-stone-400"}`, children: task.completed && /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("i", { className: "fa-solid fa-check text-white text-[10px]" }) }) }),
              /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "flex-1 min-w-0", children: [
                /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "flex justify-between items-center", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "flex-1", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("p", { className: `text-sm font-medium transition-colors ${task.completed ? "text-stone-300" : "text-stone-700"}`, children: task.description }),
                    /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "flex items-center gap-4 mt-1.5 opacity-40 group-hover:opacity-100 transition-opacity", children: [
                      task.skillTag && /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("span", { className: "text-[9px] uppercase tracking-widest font-bold text-stone-500", children: task.skillTag }),
                      task.xpReward > 0 && !task.completed && /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("span", { className: "text-[9px] text-stone-500 font-mono", children: [
                        task.xpReward,
                        " XP"
                      ] })
                    ] })
                  ] }),
                  isHabit && /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("div", { className: "pl-4", children: /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("span", { className: `text-xs font-mono font-medium ${task.completed ? "text-stone-300" : "text-stone-500"}`, children: [
                    task.currentValue,
                    /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("span", { className: "text-stone-300 mx-0.5", children: "/" }),
                    task.targetValue
                  ] }) })
                ] }),
                isHabit && task.targetValue && !task.completed && (task.currentValue || 0) > 0 && /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("div", { className: "w-full bg-stone-50 h-[3px] mt-3 rounded-full overflow-hidden", children: /* @__PURE__ */ (0, import_jsx_runtime8.jsx)(
                  motion.div,
                  {
                    initial: { width: 0 },
      ...
      tasks.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("div", { className: "py-16 text-center", children: [
        /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("i", { className: "fa-solid fa-wind mb-4 text-stone-100 text-3xl" }),
        /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("p", { className: "text-stone-400 text-sm font-serif italic", children: "The list is empty. Stillness remains." })
      ] })
    ] })
  ] });
};
...
function useOnMount(callback) {
  (0, import_react24.useEffect)(callback, []);
}
...
function useSound(src, _temp) {
  ...
  var HowlConstructor = import_react24.default.useRef(null);
  var isMounted = import_react24.default.useRef(false);
  var _React$useState = import_react24.default.useState(null), duration = _React$useState[0], setDuration = _React$useState[1];
  var _React$useState2 = import_react24.default.useState(null), sound = _React$useState2[0], setSound = _React$useState2[1];
  ...
  useOnMount(function() {
    Promise.resolve().then(() => __toESM(require_howler())).then(function(mod) {
      if (!isMounted.current) {
        var _mod$Howl;
  ...
  var play = import_react24.default.useCallback(function(options) {
  ...
  var stop = import_react24.default.useCallback(function(id3) {
    if (!sound) {
      return;
    }
    sound.stop(id3);
  }, [sound]);
  var pause = import_react24.default.useCallback(function(id3) {
    if (!sound) {
      return;
    }
    sound.pause(id3);
...
var SOUNDS = {
  ...
  // Purchase
  GONG: "https://cdn.freesound.org/previews/56/56364_94921-lq.mp3"
  // Level Up / Milestone
};
var useGameSound = (enabled) => {
  const [playChime] = use_sound_esm_default(SOUNDS.CHIME, { volume: 0.5 });
  const [playCoin] = use_sound_esm_default(SOUNDS.COIN, { volume: 0.5 });
  const [playGong] = use_sound_esm_default(SOUNDS.GONG, { volume: 0.4 });
  const play = (0, import_react25.useCallback)((sound) => {
    if (!enabled)
      return;
    switch (sound) {
      case "chime":
...
// components/DialogueOverlay.tsx
var import_jsx_runtime9 = __toESM(require_jsx_runtime());
var DialogueOverlay = ({ isOpen, speakerName, text, onClose, onNext }) => {
  return /* @__PURE__ */ (0, import_jsx_runtime9.jsx)(AnimatePresence, { children: isOpen && /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)(
    motion.div,
    {
      ...
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("div", { className: "absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent pointer-events-auto", onClick: onClose }),
        /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)(
          motion.div,
          {
            ...
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("div", { className: "absolute -top-6 left-6 bg-stone-800 text-white px-4 py-1 rounded-t-lg font-bold tracking-widest uppercase text-sm border-t-2 border-l-2 border-r-2 border-stone-800", children: speakerName }),
              /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)("p", { className: "text-xl font-serif text-stone-900 leading-relaxed mb-4", children: [
                '"',
                text,
                '"'
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)("div", { className: "flex justify-end gap-4", children: [
                /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("button", { onClick: onClose, className: "text-xs font-bold uppercase tracking-widest text-stone-400 hover:text-stone-600 transition-colors", children: "Dismiss" }),
                /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)("button", { onClick: onNext || onClose, className: "text-xs font-bold uppercase tracking-widest bg-stone-800 text-white px-4 py-2 rounded hover:bg-stone-900 transition-colors animate-pulse", children: [
                  "Next ",
                  /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("i", { className: "fa-solid fa-caret-right ml-1" })
                ] })
              ] })
            ]
          }
...
var DialogueOverlay_default = DialogueOverlay;

// components/WeatherOverlay.tsx
var import_jsx_runtime10 = __toESM(require_jsx_runtime());
var PETALS = Array.from({ length: 12 }).map((_, i) => ({
  id: i,
...
var WeatherOverlay = () => {
  return /* @__PURE__ */ (0, import_jsx_runtime10.jsx)("div", { className: "absolute inset-0 pointer-events-none overflow-hidden z-0", children: PETALS.map((petal) => /* @__PURE__ */ (0, import_jsx_runtime10.jsx)(
    motion.div,
    {
...
var WeatherOverlay_default = WeatherOverlay;

// components/HomeBase.tsx
...
var SPRITE_MAP = {
  ...
  "DOG_TRAINING": "sprite_companion_dog_1766253672506.png",
  // Show dog instead
  ...
  "SCOUT": "sprite_spirit_wisp_1766253698411.png"
  // Show wisp
};
var PixelAvatar = ({ action, scale: scale2 = 1, assetBasePath }) => {
  const spriteFile = SPRITE_MAP[action] || SPRITE_MAP["IDLE"];
  const [imgError, setImgError] = import_react26.default.useState(false);
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime11.jsxs)(
    "div",
    {
      ...
      style: {
        width: 120 * scale2,
        // Increased base size for 4k assets
        height: 120 * scale2,
        imageRendering: "pixelated"
      },
      children: [
        !imgError ? /* @__PURE__ */ (0, import_jsx_runtime11.jsx)(
          "img",
          {
            src: imgSrc,
            alt: action,
          ...
          /* @__PURE__ */ (0, import_jsx_runtime11.jsx)("div", { className: "w-20 h-20 bg-indigo-400 rounded-lg flex items-center justify-center shadow-lg animate-pulse", children: /* @__PURE__ */ (0, import_jsx_runtime11.jsx)("span", { className: "text-white text-xs font-bold uppercase", children: action }) })
        ),
        !imgError && /* @__PURE__ */ (0, import_jsx_runtime11.jsx)("div", { className: "absolute bottom-2 left-1/2 -translate-x-1/2 w-1/2 h-2 bg-black/20 rounded-full blur-[2px]" }),
...
var PixelAvatar_default = PixelAvatar;
...
var TILE_HEIGHT = 32;
var BLOCK_HEIGHT = 16;
var getIsoCoords = (x, y) => {
  const isoX = (x - y) * (TILE_WIDTH / 2);
  const isoY = (x + y) * (TILE_HEIGHT / 2);
  return { x: isoX, y: isoY };
};
var getTerrainStyle = (t) => {
  switch (t) {
    case "GRASS":
      return { top: "var(--background-modifier-success)", side: "#2f4018" };
    case "WATER":
      return { top: "#356877", side: "#1b363d" };
    case "SAND":
      return { top: "#d4b859", side: "#8a7638" };
    case "MOUNTAIN":
      return { top: "#585858", side: "#2b2b2b" };
    case "SNOW":
      return { top: "#e0f0ff", side: "#a3b8cc" };
    default:
      return { top: "var(--background-primary-alt)", side: "var(--background-modifier-border)" };
  }
};
var HomeBase = ({ gameState, backgroundImage, avatarAction = "IDLE", assetBasePath }) => {
  const [hoveredCell, setHoveredCell] = (0, import_react27.useState)(null);
  const terrainMap = (0, import_react27.useMemo)(() => {
    const map = [];
    const noise = (nx, ny) => Math.sin(nx) * Math.cos(ny);
  ...
  const getItemAt = (x, y) => gameState.baseLayout.find((i) => i.x === x && i.y === y);
  return /* @__PURE__ */ (0, import_jsx_runtime12.jsxs)("div", { className: "h-[600px] w-full relative overflow-hidden bg-[var(--background-primary)] rounded-3xl border border-[var(--background-modifier-border)] shadow-inner group", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute inset-0 flex items-center justify-center p-20", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)(
      "div",
      {
        ...
        children: Array.from({ length: GRID_SIZE * GRID_SIZE }).map((_, i) => {
          ...
          return /* @__PURE__ */ (0, import_jsx_runtime12.jsxs)(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ (0, import_jsx_runtime12.jsxs)("div", { className: "relative w-full h-full", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime12.jsx)(
                    "div",
                    {
                ...
                item && /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute bottom-[20%] left-1/2 -translate-x-1/2 flex justify-center items-end z-50", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("i", { className: `fa-solid ${((_a = BUILDINGS.find((b) => b.id === item.buildingId)) == null ? void 0 : _a.icon) || "fa-question"} text-2xl text-[var(--text-normal)] opacity-80` }) }),
                x === 12 && y === 12 && /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute bottom-[20%] left-1/2 -translate-x-1/2 flex justify-center items-end z-50", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "scale-60 origin-bottom", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)(PixelAvatar_default, { action: avatarAction, assetBasePath }) }) }),
                x === 13 && y === 11 && /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute bottom-[20%] left-1/2 -translate-x-1/2 flex justify-center items-end z-40", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "scale-50 origin-bottom", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)(PixelAvatar_default, { action: "DOG", assetBasePath }) }) }),
                x === 10 && y === 14 && /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute bottom-[40%] left-1/2 -translate-x-1/2 flex justify-center items-end z-40", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "scale-40 origin-bottom animate-bounce-slow", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsx)(PixelAvatar_default, { action: "SPIRIT", assetBasePath }) }) })
              ]
            },
            `${x}-${y}`
          );
    ...
    /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "absolute bottom-8 left-8 right-8 flex justify-between items-end pointer-events-none", children: /* @__PURE__ */ (0, import_jsx_runtime12.jsxs)("div", { className: "p-6 bg-[var(--background-secondary)]/40 backdrop-blur-md rounded-2xl border border-[var(--background-modifier-border)] pointer-events-auto", children: [
      /* @__PURE__ */ (0, import_jsx_runtime12.jsxs)("h3", { className: "text-xl font-serif italic text-[var(--text-normal)] mb-1", children: [
        "Sanctuary Level ",
        Math.floor(gameState.stats.level)
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime12.jsx)("div", { className: "text-xs text-[var(--text-muted)] tracking-widest uppercase", children: "Current Zone: The Clearing" })
    ] }) })
  ] });
};
var HomeBase_default = HomeBase;
...
var ErrorBoundary = class extends import_react28.Component {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return /* @__PURE__ */ (0, import_jsx_runtime13.jsxs)("div", { className: "p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg m-4", children: [
        /* @__PURE__ */ (0, import_jsx_runtime13.jsx)("h2", { className: "font-bold mb-2", children: "Something went wrong." }),
        /* @__PURE__ */ (0, import_jsx_runtime13.jsx)("details", { className: "whitespace-pre-wrap text-xs font-mono", children: this.state.error && this.state.error.toString() })
      ] });
    }
    return this.props.children;
  }
...
var NAV_ITEMS = [
  { tab: "TASKS" /* TASKS */, label: "Journal", img: "fas fa-book" },
  { tab: "DASHBOARD" /* DASHBOARD */, label: "Garden", img: "fas fa-tree" },
  { tab: "FOCUS" /* FOCUS */, label: "Focus", img: "fas fa-clock" },
  { tab: "STORY" /* STORY */, label: "Journey", img: "fas fa-scroll" },
  { tab: "HISTORY" /* HISTORY */, label: "Chronicle", img: "fas fa-calendar-days" }
];
var POMODORO_DEFAULT = 25 * 60;
var ZEN_BG = "bg-[var(--background-primary)]";
var ZEN_TEXT = "text-[var(--text-normal)]";
var App = ({ app, plugin }) => {
  var _a, _b;
  const [gameState, setGameState] = (0, import_react29.useState)(null);
  const [persistentData, setPersistentData] = (0, import_react29.useState)(null);
  const [activeTab, setActiveTab] = (0, import_react29.useState)("TASKS" /* TASKS */);
  const [showDialogue, setShowDialogue] = (0, import_react29.useState)(false);
  const [timerActive, setTimerActive] = (0, import_react29.useState)(false);
  const [timerSeconds, setTimerSeconds] = (0, import_react29.useState)(POMODORO_DEFAULT);
  const [timerMode, setTimerMode] = (0, import_react29.useState)("POMODORO");
  const [breathPhase, setBreathPhase] = (0, import_react29.useState)("INHALE");
  const [defaultXpReward, setDefaultXpReward] = (0, import_react29.useState)(10);
  const [soundEnabled, setSoundEnabled] = (0, import_react29.useState)(true);
  const { play } = useGameSound(soundEnabled);
  const [bgUrl, setBgUrl] = (0, import_react29.useState)(void 0);
  const [assetBasePath, setAssetBasePath] = (0, import_react29.useState)("");
  ...
  (0, import_react29.useEffect)(() => {
    if (!persistentData)
      return;
    const triggerRecalc = (0, import_obsidian.debounce)(async () => {
      console.log("LifeRPG: Recalculating Stats...");
      const files = app.vault.getMarkdownFiles();
      const activeFile = app.workspace.getActiveFile();
      const relevantFiles = files.filter(
        (f) => f.path.includes("Daily/") || f.path.includes("Journal/") || f.path === (activeFile == null ? void 0 : activeFile.path)
      );
    ...
    }, persistentData.settings.debounceDelay || 2e3, true);
  ...
  if (!gameState)
    return /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: `flex items-center justify-center h-screen ${ZEN_BG} ${ZEN_TEXT}`, children: "Loading Sanctuary..." });
  return /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: `flex flex-col h-full font-serif ${ZEN_BG} ${ZEN_TEXT} overflow-hidden selection:bg-[var(--text-selection)] relative`, children: [
    /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(WeatherOverlay_default, {}),
    /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "flex-1 overflow-y-auto relative scroll-smooth", children: [
      /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "h-20 flex items-center justify-between px-12 sticky top-0 z-30 pointer-events-none", children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "text-xl italic text-[var(--text-muted)] font-light pointer-events-auto", children: "Cultivate your inner garden." }) }),
      /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "pb-32 min-h-full", children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(ErrorBoundary, { children: /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "max-w-7xl mx-auto px-4 sm:px-8", children: [
        activeTab === "TASKS" /* TASKS */ && /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "space-y-12 animate-fade-in py-12", children: [
          /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "flex flex-col items-center mb-12", children: [
            /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("h1", { className: "text-5xl font-serif text-[var(--text-normal)] mb-2 italic", children: "the chronicle" }),
            /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("p", { className: "text-[var(--text-muted)] tracking-[0.2em] uppercase text-xs", children: new Date().toLocaleDateString(void 0, { weekday: "long", month: "long", day: "numeric" }) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
            TaskList_default,
            {
              ...
              onToggleTask: (id3) => console.log("Toggle", id3)
            }
          )
        ] }),
        activeTab === "DASHBOARD" /* DASHBOARD */ && /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "h-full w-full relative", children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
          HomeBase_default,
          {
            gameState,
        ...
        activeTab === "FOCUS" /* FOCUS */ && /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "flex flex-col items-center justify-center min-h-[60vh] space-y-12 animate-fade-in py-12", children: [
          /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "text-9xl font-mono tracking-tighter text-[var(--text-normal)] opacity-80", children: formatTime(timerSeconds) }),
          /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "flex flex-col items-center gap-8", children: [
            /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
              "button",
              {
                ...
                children: timerActive ? "pause" : "begin"
              }
            ),
            /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "text-sm font-mono text-[var(--text-muted)] tracking-widest uppercase", children: [
              /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("i", { className: "fas fa-bolt text-yellow-500 mr-2" }),
              "Potential: +50 Focus XP"
            ] })
          ] })
        ] }),
        activeTab === "STORY" /* STORY */ && /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "space-y-12 animate-fade-in py-12", children: [
          /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "flex flex-col items-center mb-12", children: [
            /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("h1", { className: "text-5xl font-serif text-[var(--text-normal)] mb-2 italic", children: "the journey" }),
            /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("p", { className: "text-[var(--text-muted)] tracking-[0.2em] uppercase text-xs", children: [
              "Level ",
              ((_b = (_a = gameState == null ? void 0 : gameState.skills) == null ? void 0 : _a.find((s) => s.id === "SCHOLAR")) == null ? void 0 : _b.level) || 1,
              " Scholar"
            ] })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "p-8 bg-[var(--background-secondary)] rounded-xl border border-[var(--background-modifier-border)] text-center", children: [
            /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("p", { className: "text-[var(--text-muted)] italic", children: '"The path reveals itself only as you walk it."' }),
            /* @__PURE__ */ (0, import_jsx_runtime14.jsxs)("div", { className: "mt-4 p-4 bg-[var(--background-primary)] rounded border border-[var(--background-modifier-border)]", children: [
              /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("h3", { className: "font-bold text-[var(--text-normal)]", children: "Chapter 1: The Awakeining" }),
              /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("p", { className: "text-sm text-[var(--text-muted)] mt-2", children: "You have begun to document your life. The fog lifts." })
            ] })
          ] })
        ] }),
        activeTab === "HISTORY" /* HISTORY */ && /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "animate-fade-in py-12", children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
          Chronicle_default,
          {
            stats: (persistentData == null ? void 0 : persistentData.history) || {},
            onDayClick: (date) => console.log("Clicked", date)
          }
        ) })
      ] }) }) })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "fixed bottom-10 left-1/2 -translate-x-1/2 z-50", children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("div", { className: "bg-[var(--background-secondary)]/80 backdrop-blur-xl border border-[var(--background-modifier-border)] px-3 py-2 rounded-[24px] shadow-xl flex items-center gap-1", children: NAV_ITEMS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
      "button",
      {
        ...
        className: `flex flex-col items-center justify-center w-14 h-14 rounded-[18px] transition-all duration-500 group relative
                                ${activeTab === item.tab ? "bg-[var(--interactive-accent)] text-white shadow-lg scale-110" : "text-[var(--text-muted)] hover:bg-[var(--background-modifier-hover)]"}`,
        children: /* @__PURE__ */ (0, import_jsx_runtime14.jsx)("i", { className: `fa-solid ${item.img} text-lg mb-1 ${activeTab !== item.tab ? "text-stone-500" : "text-white"}` })
      },
      item.tab
    )) }) }),
    /* @__PURE__ */ (0, import_jsx_runtime14.jsx)(
      DialogueOverlay_default,
      {
        isOpen: showDialogue,
        speakerName: "Scout",
...
var ReviewQueue = ({ unknownSources, onMap, onIgnore }) => {
  const entries = Object.entries(unknownSources || {}).sort((a, b) => b[1] - a[1]);
  const [mappingKey, setMappingKey] = import_react30.default.useState(null);
  const [selectedSkill, setSelectedSkill] = import_react30.default.useState("INTELLECT");
  const [xpAmount, setXpAmount] = import_react30.default.useState(10);
  ...
  if (entries.length === 0) {
    return /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "p-4 bg-[var(--background-modifier-success)]/10 border border-[var(--background-modifier-success)] rounded-lg text-sm text-[var(--text-success)]", children: [
      /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("i", { className: "fa-solid fa-check-circle mr-2" }),
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "space-y-4", children: [
    /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "flex items-center justify-between", children: [
      /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("h3", { className: "text-lg font-serif italic", children: [
        "Unmapped Data (",
        entries.length,
        ")"
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("span", { className: "text-xs text-[var(--text-muted)]", children: "Found in your Daily Notes" })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("div", { className: "grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar", children: entries.map(([key, count]) => /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "flex flex-col gap-2 p-3 bg-[var(--background-primary)] rounded-lg border border-[var(--background-modifier-border)]", children: [
      /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "flex items-center justify-between", children: [
        /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("div", { className: "font-bold text-[var(--text-normal)]", children: key }),
          /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "text-xs text-[var(--text-muted)]", children: [
            "Found ",
            count,
            " times"
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("div", { className: "flex gap-2", children: mappingKey === key ? /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("button", { onClick: () => setMappingKey(null), className: "text-[var(--text-muted)] hover:text-[var(--text-normal)]", children: /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("i", { className: "fa-solid fa-xmark" }) }) : /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)(import_jsx_runtime15.Fragment, { children: [
          /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("button", { onClick: () => setMappingKey(key), className: "px-3 py-1 text-xs bg-[var(--interactive-accent)] text-white rounded hover:opacity-90 transition-opacity", children: "Map" }),
          /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("button", { onClick: () => onIgnore(key), className: "px-3 py-1 text-xs bg-[var(--background-modifier-error)]/20 text-[var(--text-error)] rounded hover:bg-[var(--background-modifier-error)]/30 transition-colors", children: "Ignore" })
        ] }) })
      ] }),
      mappingKey === key && /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)("div", { className: "flex items-center gap-2 mt-2 bg-[var(--background-secondary)] p-2 rounded animate-fade-in", children: [
        /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("i", { className: "fa-solid fa-arrow-right text-[var(--text-muted)]" }),
        /* @__PURE__ */ (0, import_jsx_runtime15.jsxs)(
          "select",
          {
            ...
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("option", { value: "INTELLECT", children: "Intellect" }),
              /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("option", { value: "VITALITY", children: "Vitality" }),
              /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("option", { value: "CREATIVITY", children: "Creativity" }),
              /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("option", { value: "DISCIPLINE", children: "Discipline" }),
              /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("option", { value: "CONNECTION", children: "Connection" })
            ]
          }
        ),
        ...
        /* @__PURE__ */ (0, import_jsx_runtime15.jsx)("span", { className: "text-[10px] text-[var(--text-muted)] uppercase", children: "XP" }),
        /* @__PURE__ */ (0, import_jsx_runtime15.jsx)(
          "button",
          {
...
var ReviewQueue_default = ReviewQueue;

// components/Settings.tsx
var import_jsx_runtime16 = __toESM(require_jsx_runtime());
var Settings = ({ defaultXp, soundEnabled, themeMode = "SMART", manualThemeId = "classic", vaultMappings = [], customMappings = {}, unknownSources = {}, debounceDelay = 2e3, onSave, assetBasePath }) => {
  const [localXp, setLocalXp] = (0, import_react31.useState)(defaultXp);
  const [localSound, setLocalSound] = (0, import_react31.useState)(soundEnabled);
  const [localThemeMode, setLocalThemeMode] = (0, import_react31.useState)(themeMode);
  const [localManualThemeId, setLocalManualThemeId] = (0, import_react31.useState)(manualThemeId);
  const [localMappings, setLocalMappings] = (0, import_react31.useState)(vaultMappings);
  const [localCustomMappings, setLocalCustomMappings] = (0, import_react31.useState)(customMappings);
  const [localDebounce, setLocalDebounce] = (0, import_react31.useState)(debounceDelay);
  const [showSaved, setShowSaved] = (0, import_react31.useState)(false);
  const [newPattern, setNewPattern] = (0, import_react31.useState)("");
  const [newType, setNewType] = (0, import_react31.useState)("TAG");
  const [newSkill, setNewSkill] = (0, import_react31.useState)(SKILL_DEFINITIONS[0].id);
  const handleSave = () => {
    onSave({
      defaultXp: localXp,
  ...
  const removeMapping = (id3) => {
    setLocalMappings(localMappings.filter((m) => m.id !== id3));
  };
  const handleMapSource = (sourceKey, skillId, xp) => {
    setLocalCustomMappings({
      ...localCustomMappings,
      [sourceKey]: { skillId, xpPerUnit: xp, type: "COUNT" }
      // Defaulting to COUNT for now
    });
  };
  const handleIgnoreSource = (sourceKey) => {
    setLocalCustomMappings({
      ...localCustomMappings,
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "max-w-xl mx-auto space-y-8 pb-12", children: [
    /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "text-center", children: [
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("h2", { className: "text-3xl font-light text-[var(--text-normal)] mb-2", children: "The Shrine" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "text-[var(--text-muted)] italic font-serif", children: '"Adjust your path as the seasons change."' })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "bg-[var(--background-primary-alt)] p-8 rounded-xl border border-[var(--background-modifier-border)] shadow-sm space-y-8", children: [
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center justify-between mb-4", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-palette text-[var(--text-muted)]" }),
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("label", { className: "text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]", children: "Garden Theme" })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex bg-[var(--background-modifier-form-field)] rounded-lg p-1", children: [
        ...
        localThemeMode === "SMART" && /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "p-4 bg-[var(--background-secondary)] rounded-lg border border-[var(--background-modifier-border)] text-sm text-[var(--text-muted)] italic flex items-center justify-center gap-2", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-wand-magic-sparkles" }),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("span", { children: "Theme changes with time of day and season." })
        ] }),
        localThemeMode === "MANUAL" && /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "grid grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar", children: GARDEN_THEMES.map((theme) => /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)(
          "button",
          {
            onClick: () => setLocalManualThemeId(theme.id),
            className: `flex items-center gap-3 p-3 rounded-lg border text-left transition-all ${localManualThemeId === theme.id ? "bg-[var(--interactive-accent)]/10 border-[var(--interactive-accent)]" : "bg-[var(--background-secondary)] border-[var(--background-modifier-border)] hover:border-[var(--text-muted)]"}`,
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: `w-8 h-8 rounded-md bg-cover bg-center shrink-0`, style: { backgroundImage: `url("${assetBasePath}/${theme.file}")` } }),
              /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "min-w-0", children: [
                /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: `text-xs font-bold truncate ${localManualThemeId === theme.id ? "text-[var(--interactive-accent)]" : "text-[var(--text-normal)]"}`, children: theme.name }),
                /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "text-[10px] text-[var(--text-muted)] uppercase tracking-wider", children: theme.style })
              ] })
            ]
          },
          theme.id
      ...
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("hr", { className: "border-[var(--background-modifier-border)]" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center justify-between", children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center gap-4", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "w-10 h-10 rounded-full bg-[var(--background-secondary)] text-[var(--text-muted)] flex items-center justify-center", children: /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: `fas ${localSound ? "fa-volume-high" : "fa-volume-xmark"}` }) }),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "font-bold text-[var(--text-normal)]", children: "Sound Effects" }),
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "text-xs text-[var(--text-muted)]", children: "Enable ambient sounds and alerts" })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
          "button",
          {
            ...
            children: /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: `absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${localSound ? "left-7" : "left-1"}` })
          }
        )
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("hr", { className: "border-[var(--background-modifier-border)]" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center justify-between mb-4", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("label", { className: "text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]", children: "Vault Connections" }),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "text-xs text-[var(--text-muted)] italic", children: "Map folders/tags to Skills" })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "space-y-2 mb-4", children: localMappings.map((m) => /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center justify-between bg-[var(--background-secondary)] p-3 rounded-lg border border-[var(--background-modifier-border)]", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("span", { className: `text-[10px] font-bold px-1.5 py-0.5 rounded ${m.type === "TAG" ? "bg-indigo-100/10 text-indigo-400" : "bg-amber-100/10 text-amber-400"}`, children: m.type }),
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("span", { className: "font-mono text-sm text-[var(--text-normal)]", children: m.pattern }),
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-arrow-right text-[var(--text-muted)] text-xs" }),
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("span", { className: "font-bold text-[var(--text-normal)] text-sm capitalize", children: m.skillId })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("button", { onClick: () => removeMapping(m.id), className: "text-[var(--text-muted)] hover:text-red-500", children: /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-trash" }) })
        ] }, m.id)) }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex gap-2 p-2 bg-[var(--background-secondary)] rounded-lg", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)(
            "select",
            {
              ...
              children: [
                /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("option", { value: "TAG", children: "TAG (#)" }),
                /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("option", { value: "FOLDER", children: "FOLDER (/)" })
              ]
            }
          ),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
          ...
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
            "select",
            {
              ...
              children: Object.values(SKILL_DEFINITIONS).map((s) => /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("option", { value: s.id, children: s.name }, s.id))
            }
          ),
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
            "button",
            {
              ...
              children: /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-plus" })
            }
          )
        ] })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("hr", { className: "border-[var(--background-modifier-border)]" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("label", { className: "block text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2", children: "Performance Delay" }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center gap-4", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
            "input",
            {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "w-20 text-center font-mono font-bold text-[var(--text-normal)] text-sm border border-[var(--background-modifier-border)] rounded px-2 py-1", children: [
            (localDebounce / 1e3).toFixed(1),
            "s"
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("p", { className: "text-xs text-[var(--text-muted)] mt-2", children: "Adjust wait time after typing before updating stats. (Default: 2.0s)" })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("hr", { className: "border-[var(--background-modifier-border)]" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("label", { className: "block text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2", children: "Default Task XP" }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "flex items-center gap-4", children: [
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
            "input",
            {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "w-12 text-center font-mono font-bold text-[var(--text-normal)] text-lg border border-[var(--background-modifier-border)] rounded px-2 py-1", children: localXp })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("p", { className: "text-xs text-[var(--text-muted)] mt-2", children: "XP awarded for generic checked checkboxes." })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("hr", { className: "border-[var(--background-modifier-border)]" }),
      /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("label", { className: "block text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4", children: "Data Review Queue" }),
        /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
          ReviewQueue_default,
          {
      ...
      /* @__PURE__ */ (0, import_jsx_runtime16.jsx)(
        "button",
        {
          ...
          children: showSaved ? /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("span", { className: "flex items-center justify-center gap-2 text-white/80", children: [
            /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("i", { className: "fas fa-check" }),
            " Changes Saved"
          ] }) : "Save Configuration"
        }
      )
...
// main.ts
var VIEW_TYPE_LIFE_RPG = "life-rpg-view";
var LifeRPGSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    const init = async () => {
      ...
      (_a = this.root) == null ? void 0 : _a.render(
        React10.createElement(Settings_default, {
...
var LifeRPGView = class extends import_obsidian2.ItemView {
  ...
  async onOpen() {
    ...
    this.root.render(
      React10.createElement(App_default, {
        app: this.app,
        plugin: this.plugin
      })
    );
...
var LifeRPGPlugin = class extends import_obsidian2.Plugin {
  async onload() {
    this.registerView(
      VIEW_TYPE_LIFE_RPG,
      (leaf) => new LifeRPGView(leaf, this)
    );
    this.addSettingTab(new LifeRPGSettingTab(this.app, this));
    this.addRibbonIcon("sword", "Life RPG", () => {
      this.activateView();
    });
    this.addCommand({
      id: "open-life-rpg",
